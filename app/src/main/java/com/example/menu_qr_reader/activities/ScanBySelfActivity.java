...
public class ScanBySelfActivity extends AppCompatActivity {
    private ActivityResultLauncher<ScanOptions> barcodeLauncher;
    private JSONObject response;

    private ScanOptions setScanOption(){
        ScanOptions scanOptions = new ScanOptions();
        scanOptions.setCameraId(1);  // use behind camera, 1 -> front camera
    ...
    private void succeedScanning(ScanIntentResult result) {
        ...
        if (isValid) builder = builder.setTitle("Qr인증 성공").setMessage(String.format("[%s] 식권 입니다.\n%s님 맛있는 식사 되세요!", ticketName, ownerName));  // 학생 메뉴, 추가 메뉴, 교직원 메뉴
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_scan_by_self);
